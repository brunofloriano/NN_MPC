...
%for j = 10:10:200
 ...
 for j = 1:1
    ...
    %Kdata = [0 0 0 0];
    % DATA INITIALIZATION
    %K = K + DATA_MINING*[Kvector(1:2,j)';Kvector(3:4,j)']; Este campo é
    %para aprendizado offline
    state(:,1) = xaug;
    ...
    for i = 2:length(t)
        ...
        % MPC - Choosing performance function
        if MPC == 1
            eye_matrix = eye(S);
            vstate(:,1) = state(:,i);
            ...
            for vtime = 1:horizon
                ...
                if vtime == 1
                    ...
                    Jmean(vtime) = sum(Jinst(vtime,:).*Psi(vmode(vtime-1),:));
